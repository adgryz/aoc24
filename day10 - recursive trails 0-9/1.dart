...
void moveOnTrail(Point position, List<List<int>> board, maxX, int maxY, int value, Point prev, Point initialPosition) {
  ...
  possibleMoves
    ...
    if (board[move.y][move.x] == 9) {
      var trailId = initialPosition.toString() + move.toString();
      counts.add(trailId);
      // print('Found top hill at ' + move.toString());
    } else {
      // print('Move to [' + board[move.y][move.x].toString() + '] ' + move.toString() + ' from ' + position.toString());
