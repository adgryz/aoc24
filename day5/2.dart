...
void main() async {
  try {
    ...
    var correctedPrints = incorrectPrints
        .map((incorrectPrint) =>
            sortWithRules(incorrectPrint, preceedingMap, followingMap))
        .toList();

    result = correctedPrints
...
List<int> sortWithRules(List<int> incorrectPrint,
    Map<int, Set<int>> preceedingMap, Map<int, Set<int>> followingMap) {
  int compareNumbers(int a, int b) {
    if (preceedingMap[a]!.contains(b)) return 1;
    if (followingMap[a]!.contains(b)) return -1;
    return 0;
  }

  incorrectPrint.sort(compareNumbers);
